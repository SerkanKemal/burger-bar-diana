...
За временно тестване извън работното време добави случаен `TEST_ORDER_TOKEN` в Render и отвори:

```text
https://burger-bar-diana.onrender.com/?testOrder=СЪЩИЯТ_TEST_ORDER_TOKEN
```

Този режим работи само с тестов Stripe secret key и позволява единствено картови поръчки. След теста изтрий `TEST_ORDER_TOKEN` от Render.
